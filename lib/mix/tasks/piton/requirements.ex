defmodule Mix.Tasks.Piton.Requirements do
  ...
  def run(args) do
    case args do
      [pip_path, requirements_path] ->
        get_requirements(pip_path, requirements_path)

      _ ->
        IO.puts(
          "Wrong number of arguments. It can be 1 or 2. The first one has to be the folder and the second one the python interpreter."
        )
    end
  end
